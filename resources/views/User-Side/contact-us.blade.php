<x-layout>
    <?php $page = "Contact Us"; ?>
    <x-breadcrumb :page="$page" />






    <div id="main-wrapper">
        <div class="site-wrapper-reveal">

            <div class="contact-us-page-area section-space--pt_70 section-space--pb_100">
                <div class="container">
                    <div class="row">
                        <div class="col-lg-4 col-md-6 col-12">
                            <div class="single-office-info-wrap" data-aos="fade-up">
                                <a href="#" class="office-img">
                                    <img src="images/banners/office-01.jpg" alt="">
                                    <span class="office-title">Head Quater</span>
                                </a>
                                <div class="single-office-info">
                                    <div class="single-contact-info">
                                        <span class="icon">
                                        <img src="images/svg/calling.svg" alt="">
                                    </span>
                                        <span class="contact-text"><a href="#">(00) 359 467 9434</a></span>
                                    </div>
                                    <div class="single-contact-info">
                                        <span class="icon">
                                        <img src="images/icons/emaill-contact.png" alt="">
                                    </span>
                                        <span class="contact-text"><a href="#">infoofbunzon@gmial.com</a></span>
                                    </div>
                                    <div class="single-contact-info">
                                        <span class="icon">
                                        <img src="images/icons/map-contact.png" alt="">
                                    </span>
                                        <span class="contact-text">845 Central Ave
                                        Hamilton, Ohio(OH), 45011</span>
                                    </div>
                                    <div class="share-wrap mt-30">
                                        <h4 class="title">Connect With Us:</h4>
                                        <ul class="social-share-area mt-15 mb-15">
                                            <li><a href="#"><i class="icofont-facebook"></i></a></li>
                                            <li><a href="#"><i class="icofont-skype"></i></a></li>
                                            <li><a href="#"><i class="icofont-twitter"></i></a></li>
                                            <li><a href="#"><i class="icofont-linkedin"></i></a></li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-4 col-md-6 col-12">
                            <div class="single-office-info-wrap" data-aos="fade-up">
                                <a href="#" class="office-img">
                                    <img src="images/banners/office-02.jpg" alt="">
                                    <span class="office-title">USA, Office</span>
                                </a>
                                <div class="single-office-info">
                                    <div class="single-contact-info">
                                        <span class="icon">
                                        <img src="images/svg/calling.svg" alt="">
                                    </span>
                                        <span class="contact-text"><a href="#">(00) 359 467 9434</a></span>
                                    </div>
                                    <div class="single-contact-info">
                                        <span class="icon">
                                        <img src="images/icons/emaill-contact.png" alt="">
                                    </span>
                                        <span class="contact-text"><a href="#">infoofbunzon@gmial.com</a></span>
                                    </div>
                                    <div class="single-contact-info">
                                        <span class="icon">
                                        <img src="images/icons/map-contact.png" alt="">
                                    </span>
                                        <span class="contact-text">845 Central Ave
                                        Hamilton, Ohio(OH), 45011</span>
                                    </div>
                                    <div class="share-wrap mt-30">
                                        <h4 class="title">Connect With Us:</h4>
                                        <ul class="social-share-area mt-15 mb-15">
                                            <li><a href="#"><i class="icofont-facebook"></i></a></li>
                                            <li><a href="#"><i class="icofont-skype"></i></a></li>
                                            <li><a href="#"><i class="icofont-twitter"></i></a></li>
                                            <li><a href="#"><i class="icofont-linkedin"></i></a></li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-4 col-md-6 col-12">
                            <div class="single-office-info-wrap" data-aos="fade-up">
                                <a href="#" class="office-img">
                                    <img src="images/banners/office-03.jpg" alt="">
                                    <span class="office-title">India, Office</span>
                                </a>
                                <div class="single-office-info">
                                    <div class="single-contact-info">
                                        <span class="icon">
                                        <img src="images/svg/calling.svg" alt="">
                                    </span>
                                        <span class="contact-text"><a href="#">(00) 359 467 9434</a></span>
                                    </div>
                                    <div class="single-contact-info">
                                        <span class="icon">
                                        <img src="images/icons/emaill-contact.png" alt="">
                                    </span>
                                        <span class="contact-text"><a href="#">infoofbunzon@gmial.com</a></span>
                                    </div>
                                    <div class="single-contact-info">
                                        <span class="icon">
                                        <img src="images/icons/map-contact.png" alt="">
                                    </span>
                                        <span class="contact-text">845 Central Ave
                                        Hamilton, Ohio(OH), 45011</span>
                                    </div>
                                    <div class="share-wrap mt-30">
                                        <h4 class="title">Connect With Us:</h4>
                                        <ul class="social-share-area mt-15 mb-15">
                                            <li><a href="#"><i class="icofont-facebook"></i></a></li>
                                            <li><a href="#"><i class="icofont-skype"></i></a></li>
                                            <li><a href="#"><i class="icofont-twitter"></i></a></li>
                                            <li><a href="#"><i class="icofont-linkedin"></i></a></li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="contact-from section-space--pt_80">
                        <div class="row align-items-center">
                            <div class="col-lg-5">
                                <div class="section-title" data-aos="fade-up">
                                    <h3 class="title mb-30">Get In Touch</h3>
                                </div>
                                <form id="#contact-form" class="contact-us-form" action="http://whizthemes.com/mail-php/jowel/bunzo/mail.php" method="post">
                                    <div class="single-input-box">
                                        <input type="text" name="con_name" placeholder="Name">
                                    </div>
                                    <div class="single-input-box">
                                        <input type="email" name="con_email" placeholder="Email">
                                    </div>
                                    <div class="single-input-box">
                                        <input type="text" name="con_phone" placeholder="Phone">
                                    </div>
                                    <div class="single-input-box">
                                        <textarea name="con_message" placeholder="Massage"></textarea>
                                    </div>
                                    <div class="single-input-box">
                                        <button type="submit" class="btn-primary btn-large">Send Message <i class="icofont-long-arrow-right"></i></button>
                                    </div>
                                    <p class="form-messege"></p>
                                </form>
                            </div>
                            <div class="col-lg-6 offset-lg-1" data-aos="fade-up">
                                <div class="contact-us-map">
                                    <iframe title="Google Map" src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3151.8402891185374!2d144.95373631590425!3d-37.81720974201477!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x6ad65d4c2b349649%3A0xb6899234e561db11!2sEnvato!5e0!3m2!1sen!2ssg!4v1607294780661!5m2!1sen!2ssg"></iframe>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

        </div>
    </div>



</x-layout>
